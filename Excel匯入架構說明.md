...
此分頁為所有計畫資料的來源，用於後續自動帶入各項表單。

### 欄位定義

| 欄位名稱 | 資料類型 | 說明 | 範例 |
|---|---|---|---|
| **計畫編號** | 文字 | 每個計畫的唯一識別碼 | P-114-001 |
| **計畫名稱** | 文字 | 計畫的完整名稱 | 從庫房到衣著，拔馬部落衣飾復刻及日常化計畫 |
| **實施地點** | 文字 | 計畫的主要執行地點 | 拔馬部落 |
| **輔導老師姓名** | 文字 | 分配給此計畫的輔導老師 | 王小明 |
| **計畫開始日期** | 日期 | 計畫的起始日期 | 2025/12/26 |
| **計畫結束日期** | 日期 | 計畫的結束日期 | 2026/12/05 |
| **OKR目標** | 文字 | 計畫的核心目標與關鍵成果 | 完成拔馬部落傳統服飾復刻，並舉辦至少兩場推廣活動。 |
...
```excel
【工作項目名稱】自動帶入公式：
=IFERROR(VLOOKUP(C6,計畫資料!$H$6:$M$20,2,FALSE),"")

說明：
- C6：對應項目 ID 欄位
- 計畫資料!$H$6:$M$20：計畫資料表的資料範圍（對應到關鍵項目相關欄位）
- 2：取得第 2 欄（工作項目名稱）
- FALSE：精確比對

【預定目標值】自動帶入公式：
=IFERROR(VLOOKUP(C6,計畫資料!$H$6:$M$20,5,FALSE),"")
...
```excel
【核定金額】自動帶入公式：
=IFERROR(SUMIF(計畫資料!$J$6:$J$20,C6,計畫資料!$K$6:$K$20),"")

說明：
- 計畫資料!$J$6:$J$20：預算科目欄位範圍
- C6：當前選擇的科目代碼
- 計畫資料!$K$6:$K$20：核定金額欄位範圍
...
```
1. 使用者下載 Excel 範本
2. 填寫「計畫資料」主表，包含計畫基本資訊、OKR、期程等
3. 每月填寫「月報表」與「經費支出」
4. 上傳至系統進行匯入
5. 系統驗證資料完整性
6. 匯入成功後，系統即可在各對應功能（如訪視紀錄）自動帶入所需資料
```
