...
import { Project, Objective, ProjectStatus, KRStatus, KeyResult, ContactInfo, BudgetItem, BudgetCategory, Vision, UserRole } from '../types';
import { Plus, Trash2, Save, ArrowLeft, PlusCircle, UserCircle, Clock, Target, Calculator, MapPin, Building, DollarSign, Users, Layers, ChevronDown } from 'lucide-react';
...
const ProjectForm: React.FC<ProjectFormProps> = ({ project, onBack, onSave, currentUserRole }) => {
  const isAdmin = currentUserRole === 'MOC_ADMIN' || currentUserRole === UserRole.ADMIN;
  ...
  const removeSite = (index: number) => {
    const newSites = (formData.sites || []).filter(s => s).filter((_, i) => i !== index);
    setFormData({ ...formData, sites: newSites });
  };
    ...
    return (
      <div className="bg-slate-50/50 p-6 rounded-3xl border border-slate-100 space-y-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          ...
          <div className="space-y-1">
            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-1">職稱</label>
            <input
              id={`${field}-title`}
              type="text"
              placeholder="請輸入職稱"
          ...
          <div className="space-y-1">
            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-1">電話</label>
            <input
              id={`${field}-phone`}
              type="tel"
          ...
          <div className="space-y-1">
            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-1">手機</label>
            <input
              id={`${field}-mobile`}
              type="tel"
        ...
        <section className="bg-white rounded-[32px] shadow-sm border border-gray-100 overflow-hidden">
          ...
          <div className="p-10 space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="space-y-2">
                <label htmlFor="start-date" className="text-xs font-black text-slate-400 uppercase tracking-widest pl-1">計畫開始日 <span className="text-red-500">*</span></label>
                <input
                  id="start-date"
                  type="date"
                  className="form-input"
              ...
              <div className="space-y-2">
                <label htmlFor="end-date" className="text-xs font-black text-slate-400 uppercase tracking-widest pl-1">計畫結束日 <span className="text-red-500">*</span></label>
                <input
                  id="end-date"
                  type="date"
                  className="form-input"
              ...
              <div className="space-y-2">
                <label htmlFor="period" className="text-xs font-black text-slate-400 uppercase tracking-widest pl-1">計畫期程（文字說明）</label>
                <input
                  id="period"
                  type="text"
                  className="form-input"
                  placeholder="例：115年1月至12月"
            ...
            <div className="space-y-4">
              ...
              <div className="p-6 bg-purple-50/50 rounded-xl border border-purple-100 space-y-5">
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex flex-col sm:flex-row gap-4 pl-6">
                    <label className="flex items-center gap-3 cursor-pointer group">
                      ...
                      />
                      <span className="text-sm font-bold text-slate-600 group-hover:text-purple-700">(1) 無涉產業及就業</span>
                    </label>
                    <label className="flex items-center gap-3 cursor-pointer group">
                      <input
                        type="radio"
                        ...
                        className="w-4 h-4 accent-purple-600"
                      />
                      <span className="text-sm font-bold text-slate-600 group-hover:text-purple-700">(2) 涉及產業及就業</span>
                    </label>
                  </div>
                </div>
                {/* 都市文化行動計畫類 */}
                ...
                {formData.category && (
                  <div className="mt-2 px-4 py-2 bg-purple-100 rounded-lg text-sm font-bold text-purple-700">
                    已選擇：{formData.category}
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="space-y-4">
                  ...
                  <div className="flex items-center gap-3">
                    <div className="relative flex-1">
                      <select
                        ...
                        value={pendingRuralSite}
                        onChange={e => { setPendingRuralSite(e.target.value); setSiteError(''); }}
                      >
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex items-center gap-3">
                    <div className="relative flex-1">
                      <select
                        className="form-input w-full appearance-none pr-10"
                        value={pendingUrbanCity}
                        onChange={e => { setPendingUrbanCity(e.target.value); setPendingUrbanDistrict(''); setSiteError(''); }}
                      >
                    ...
                    <div className="relative flex-1">
                      <select
                        className="form-input w-full appearance-none pr-10"
                        value={pendingUrbanDistrict}
                        onChange={e => { setPendingUrbanDistrict(e.target.value); setSiteError(''); }}
                        disabled={!pendingUrbanCity}
                      >
                        <option value="">{pendingUrbanCity ? '―― 選擇鄉鎮市區 ――' : '請先選縣市'}</option>
                        {(pendingUrbanCity ? getDistrictsByCity(pendingUrbanCity) : []).map(district => (
              ...
              {(formData.sites || []).filter(s => s).length > 0 && (
                <div className="mt-2 p-3 bg-slate-50 rounded-xl border border-slate-100">
                  <p className="text-xs font-black text-slate-400 mb-2">已新增地點：</p>
                  <div className="flex flex-wrap gap-2">
                    {(formData.sites || []).filter(s => s).map((site, idx) => (
                      <span key={idx} className="flex items-center gap-1 px-3 py-1 bg-white border border-slate-200 rounded-lg text-xs font-bold text-slate-700">
                        {site}
                        <button onClick={() => removeSite(idx)} className="text-red-300 hover:text-red-500 ml-1" title="移除">
                          <Trash2 size={12} />
                        </button>
                      </span>
                    ))}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="bg-blue-50/50 p-6 rounded-3xl border border-blue-100 space-y-3">
                ...
                <input
                  ...
                  value={formData.appliedAmount ? formData.appliedAmount.toLocaleString('zh-TW') : ''}
                  placeholder="0"
                  onChange={e => setFormData({ ...formData, appliedAmount: parseCurrency(e.target.value) })}
                />
                <p className="text-xs text-blue-400 font-bold">請輸入數字，系統自動加入千分位</p>
              </div>
              <div className="bg-emerald-50/50 p-6 rounded-3xl border border-emerald-100 space-y-3">
                ...
                <input
                  ...
                  value={formData.approvedAmount ? formData.approvedAmount.toLocaleString('zh-TW') : ''}
                  placeholder="0"
                  onChange={e => setFormData({ ...formData, approvedAmount: parseCurrency(e.target.value) })}
                />
                <p className="text-xs text-emerald-400 font-bold">請輸入數字，系統自動加入千分位</p>
              </div>
            </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full border-collapse">
              <thead>
                <tr className="bg-slate-50 text-[11px] font-black text-slate-400 uppercase tracking-widest text-left">
                  ...
                  <th className="px-8 py-5 text-center">單位</th>
                  <th className="px-8 py-5 text-right">單價（元）</th>
                  <th className="px-8 py-5 text-right">總價（元）</th>
                ...
                {formData.budgetItems?.map((item) => (
                  <tr key={item.id} className="hover:bg-slate-50/50">
                    ...
                    <td className="px-8 py-4 text-center">
                      <input type="number" min="1" className="w-16 text-center bg-slate-50 rounded p-1 font-mono border border-slate-200" value={item.quantity} onChange={e => updateBudgetItem(item.id, 'quantity', Number(e.target.value))} />
                    </td>
                    <td className="px-8 py-4 text-center">
                      <input type="text" className="w-16 text-center bg-slate-50 rounded p-1 border border-slate-200" placeholder="式/人/月" value={item.unit} onChange={e => updateBudgetItem(item.id, 'unit', e.target.value)} />
                    </td>
                    <td className="px-8 py-4 text-right">
                      <input type="number" className="w-28 text-right bg-slate-50 rounded p-1 font-mono border border-slate-200" value={item.unitPrice} onChange={e => updateBudgetItem(item.id, 'unitPrice', Number(e.target.value))} />
                    </td>
                    <td className="px-8 py-4 text-right">
            ...
            {(!formData.budgetItems || formData.budgetItems.length === 0) && (
              <div className="p-12 text-center text-slate-300 font-bold">
                尚未新增預算項目，請點擊右上角「新增細目」
              </div>
            )}
            {formData.budgetItems && formData.budgetItems.length > 0 && (
              <div className="p-6 bg-slate-50 border-t border-slate-100 flex justify-end">
                <div className="text-right">
                  <span className="text-xs font-black text-slate-400 uppercase tracking-widest">預算總計</span>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
              {[1, 2, 3, 4].map(stage => (
                <div key={stage} className="bg-slate-50 p-6 rounded-2xl border border-slate-100 space-y-3">
                  <label className="text-xs font-black text-slate-400 uppercase tracking-widest">第 {stage} 期款截止日</label>
                  <input
                    type="date"
